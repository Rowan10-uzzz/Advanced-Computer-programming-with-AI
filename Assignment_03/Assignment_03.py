...
class OrderItem:
    """An item in an order; composition relationship with Product."""
...
class Customer:
    """Base membership tier defining the polymorphic interface."""
    def __init__(self, name):
        if not name: raise ValueError("Customer name is required.")
        self.name = name

    def discount_rate(self, subtotal): return 0.0
    def points_multiplier(self): return 1
    def __str__(self): return "none"


class SilverCustomer(Customer):
    def discount_rate(self, subtotal): return 0.05 if subtotal > DISCOUNT_THRESHOLD else 0.02
    def points_multiplier(self): return 2
    def __str__(self): return "silver"


class GoldCustomer(Customer):
    def discount_rate(self, subtotal): return 0.10 if subtotal > DISCOUNT_THRESHOLD else 0.05
    def points_multiplier(self): return 3
    def __str__(self): return "gold"


class PlatinumCustomer(Customer):
    def discount_rate(self, subtotal): return 0.15 if subtotal > DISCOUNT_THRESHOLD else 0.10
    def points_multiplier(self): return 5
    def __str__(self): return "platinum"


def make_customer(name, tier):
    customer_classes = {"none": Customer, "silver": SilverCustomer, "gold": GoldCustomer, "platinum": PlatinumCustomer}
    try: return customer_classes[tier](name)
    except KeyError: raise ValueError("Unknown membership tier.")
...
class Order:
    ...
    def __init__(self, customer, items):
        ...
        if not items: raise ValueError("Order must contain at least one item.")
        if not all(isinstance(item, OrderItem) for item in items): raise TypeError("items must contain OrderItem objects.")
        self.customer, self.items = customer, list(items)

    def subtotal(self): return sum(item.line_total() for item in self.items)
    ...
    def tax(self): return sum(item.tax() for item in self.items)
    def total(self): return self.subtotal() - self.discount() + self.tax()
    def points(self): return int(self.total() // POINTS_DIVISOR) * self.customer.points_multiplier()

    def receipt(self):
        lines = ["Receipt for " + self.customer.name + " (" + str(self.customer) + ")", "-" * 40]
        for item in self.items:
            lines.append(item.product.name + " x" + str(item.quantity) + " = " + str(item.line_total()))
        lines.extend(["-" * 40, "Subtotal: " + str(round(self.subtotal(), 2)),
                      "Discount: " + str(round(self.discount(), 2)), "Tax: " + str(round(self.tax(), 2)),
                      "Total: " + str(round(self.total(), 2)), "Points earned: " + str(self.points()), ""])
        return "\n".join(lines)


def build_orders():
    products = [Product("Laptop",1200.0,"electronics"), Product("Headphones",200.0,"electronics"),
                Product("Coffee Beans",15.0,"food"), Product("Notebook",5.0,"stationery"),
                Product("Water Bottle",10.0,"food"), Product("Monitor",300.0,"electronics"), Product("Pen",2.0,"stationery")]
    order_data = [("Alice","gold",[(0,1),(1,2),(2,3)]),("Bob","none",[(3,10),(6,5)]),
                  ("Charlie","platinum",[(5,2),(4,6),(2,2)]),("Dana","silver",[(1,1),(3,3),(6,10)])]
    return [Order(make_customer(name,tier), [OrderItem(products[i],q) for i,q in item_data])
